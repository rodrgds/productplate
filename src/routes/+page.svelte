<script lang="ts">
	...
	const heroStack = [
		...
		{ name: 'Better Auth', color: '#111111' },
		{ name: 'Autumn', color: '#7c3aed' },
		{ name: 'AI SDK', color: '#111111' }
	] as const;
	...
	<header class="border-b bg-background/85 backdrop-blur">
		...
		>
			...
			<div class="flex items-center gap-2">
				...
				<Button href="https://github.com/rodrgds/productplate" variant="outline" size="sm" class="hidden sm:inline-flex">
					GitHub
				</Button>
			...
			>
				<div>
					...
					<p class="mt-7 max-w-xl text-lg leading-8 text-muted-foreground">
						Product Plate gives you the SaaS base: auth, billing, Convex, AI, tests, deployment,
						and app UI. The Kickstart prompt tells your coding agent what to keep, rename, and remove.
					</p>
					...
					<div class="mt-8 flex flex-wrap gap-2">
						{#each heroStack as item (item.name)}
							<span
								class="rounded-full border bg-background px-3 py-1.5 text-sm font-medium shadow-sm"
				...
				<div class="relative lg:pl-4">
					<div class="absolute -inset-3 rounded-3xl bg-gradient-to-br from-primary/20 via-transparent to-muted"></div>
					<div class="relative overflow-hidden rounded-2xl border bg-card shadow-xl shadow-foreground/5">
							...
							<div class="rounded-xl border bg-muted/35 p-4">
								<div class="flex items-center gap-2">
									...
									<div>
										...
										<p class="mt-1 text-xs text-muted-foreground">The part most templates do not ship.</p>
									</div>
								</div>
								<div class="mt-5 grid gap-2 rounded-lg bg-background p-3 font-mono text-xs leading-6">
									<p><span class="text-muted-foreground">$</span> clone productplate my-app</p>
									<p><span class="text-muted-foreground">$</span> paste START_HERE.md into your agent</p>
									<p><span class="text-muted-foreground">agent</span> asks what you are building</p>
									<p><span class="text-muted-foreground">agent</span> keeps the stack, removes the demo</p>
									<p><span class="text-muted-foreground">agent</span> leaves a smaller product repo</p>
								</div>
							</div>
			...
			<div class="mx-auto grid max-w-6xl gap-10 px-6 lg:grid-cols-[0.86fr_1.14fr] lg:items-center">
				...
				<div class="rounded-2xl border bg-card p-5 shadow-sm">
					<div class="grid gap-3">
						{#each [
							'Rename the app, constants, metadata, and README',
							'Keep only the backend, auth, billing, and AI choices that fit',
							'Remove demo account routes and unused showcase pages',
							'Update AGENTS.md so the next agent sees the real product',
							'Run the right checks and leave a clear handoff'
						] as item (item)}
